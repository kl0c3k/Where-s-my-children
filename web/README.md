...
## Multiplatform App (Android/WWW (iOS next))
...
### Demo
See latest Multiplatform App (Android/WWW (iOS next)) Demo (from master) at https://polymerelements.github.io/polymer-starter-kit/

### Tutorials

Check out the Multiplatform App (Android/WWW (iOS next)) tutorials on [polymer-project.org](https://www.polymer-project.org):
...
To take advantage of Multiplatform App (Android/WWW (iOS next)) you need to:
...
[Download](https://github.com/polymerelements/polymer-starter-kit/releases/latest) and extract Multiplatform App (Android/WWW (iOS next)) to where you want to work. The project comes in two flavours - Light and Full.

**Beginners**: Try Multiplatform App (Android/WWW (iOS next)) Light. This doesn't require any extra dependencies nor knowledge of modern front-end tooling. This option is good for prototyping if you haven't build a Polymer app before.

**Intermediate - Advanced**: Use the full version of Multiplatform App (Android/WWW (iOS next)). This comes with all the build tools you'll need for testing and productionising your app so it's nice and lean. You'll need to run a few extra commands to install the tools we recommend but it's worth it to make sure your final app is super optimised.

:warning: **Important**: Multiplatform App (Android/WWW (iOS next)), and Multiplatform App (Android/WWW (iOS next)) Light, both contain dotfiles (files starting with a `.`). If you're copying the contents of the Starter Kit to a new location make sure you bring along these dotfiles as well! On Mac, [enable showing hidden files](http://ianlunn.co.uk/articles/quickly-showhide-hidden-files-mac-os-x-mavericks/), then try extracting/copying Multiplatform App (Android/WWW (iOS next)) again. This time the dotfiles needed should be visible so you can copy them over without issues.

Rob Dodson has a fantastic [PolyCast video](https://www.youtube.com/watch?v=xz-yixRxZN8) available that walks through using Multiplatform App (Android/WWW (iOS next)). An [end-to-end with Polymer](https://www.youtube.com/watch?v=1f_Tj_JnStA) and Multiplatform App (Android/WWW (iOS next)) talk is also available.

### Install dependencies

#### Quick-start (for experienced users)

With Node.js installed, run the following one liner from the root of your Multiplatform App (Android/WWW (iOS next)) download:
...
Web apps built with Multiplatform App (Android/WWW (iOS next)) come configured with support for [Web Component Tester](https://github.com/Polymer/web-component-tester) - Polymer's preferred tool for authoring and running unit tests. This makes testing your element based applications a pleasant experience.
...
To enable Service Worker support for Multiplatform App (Android/WWW (iOS next)) project use these 3 steps:
...
If for any reason you need to disable Service Worker support after previously enabling it, you can remove it from your Multiplatform App (Android/WWW (iOS next)) project using these 4 steps:
...
4. Navigate to `chrome://serviceworker-internals` and unregister any Service Workers registered by Multiplatform App (Android/WWW (iOS next)) for your app just in case there's a copy of it cached.
...
[generator-polymer](https://github.com/yeoman/generator-polymer/releases) now includes support for Multiplatform App (Android/WWW (iOS next)) out of the box.
...
You can select a template and copy over the relevant parts you would like to reuse to Multiplatform App (Android/WWW (iOS next)).
...
Don't worry! We've got you covered. Multiplatform App (Android/WWW (iOS next)) tries to offer everything you need to build and optimize your apps for production, which is why we include the tooling we do. We realise however that our tooling setup may not be for everyone.

If you find that you just want the simplest setup possible, we recommend using Multiplatform App (Android/WWW (iOS next)) light, which is available from the [Releases](https://github.com/PolymerElements/polymer-starter-kit/releases) page. This takes next to no time to setup.

## Licensing

Like other Google projects, Multiplatform App (Android/WWW (iOS next)) includes Google license headers at the top of several of our source files. Google's open-source licensing requires that this header be kept in place (sorry!), however we acknowledge that you may need to add your own licensing to files you modify. This can be done by appending your own extensions to these headers.

## Contributing

Multiplatform App (Android/WWW (iOS next)) is a new project and is an ongoing effort by the Web Component community. We welcome your bug reports, PRs for improvements, docs and anything you think would improve the experience for other Polymer developers.
